...
class DataType:
    ...
    ##############################################################                    
    def _make_a_taxon(self,idd,age,compartment):
        taxon = ""
        taxon += f'\t<taxon id="{idd}">\n'
    ...
    def _make_taxa(self):
        ...
        if number_taxa == 0:
            taxa += '\t<!-- No taxa, is there an input error? The ages need to match the sequences. -->\n'
            print("No taxa, is there an input error? The ages need to match the sequences.")        
        taxa += '</taxa>\n'
        return taxa
    ##############################################################
    def _make_a_sequence(self,idd,seq):
        seqstr = "\t<sequence>\n"
        seqstr += f'\t\t<taxon idref="{idd}"/>\n'
        seqstr += f'\t\t{seq}\n'
        seqstr += '\t</sequence>\n'
        return seqstr
    ##############################################################    
    def _make_alignment(self):        
        ...
        number_alignmnets = 0
        for idd,seq in self.dic_taxon_seq.items():
            if idd in self.dic_taxon_age:
                algn += self._make_a_sequence(idd,seq.strip())        
                number_alignmnets += 1
            # don't use it otherwise as there is no age data        
        if number_alignmnets == 0:
            algn += '\t<!-- No alignnents, is there an input error? The ages need to match the sequences. -->\n'
            print("No alignnents, is there an input error? The ages need to match the sequences.")        
        algn += '</alignment>\n'
        return algn        
    ##############################################################
